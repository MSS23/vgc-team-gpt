...
function teamCard(team, featured = false) {
  const pokemonSlots = team.pokemon.map(p => `
    <div class="pokemon-slot">
      ${spriteImg(p.name, featured ? 80 : 64)}
  ...
  return `
    <div class="team-card ${featured ? 'featured-card' : ''}">
...
function searchTeamsHTML(teams, query, total) {
  ...
  const header = `
    <div class="header">
      ...
      <p class="subtitle">
        Showing <span class="count">${teams.length}</span> of <span class="count">${total}</span> teams for "<strong>${query}</strong>"
      </p>
    </div>
  `;

  const cards = teams.slice(0, 12).map(t => teamCard(t)).join('');

  return wrapHTML(header + `<div class="team-grid">${cards}</div>`, 'Search Results');
}
...
function rentalTeamsHTML(teams, pokemon = null, regulation = null) {
  ...
  return wrapHTML(header + `<div class="team-grid">${cards}</div>`, 'Rental Teams');
}
...
function playerTeamsHTML(player, teams) {
  ...
  return wrapHTML(header + `<div class="team-grid">${cards}</div>`, `${player} Teams`);
}
...
function tournamentTeamsHTML(event, teams) {
  ...
  return wrapHTML(header + `<div class="team-grid">${cards}</div>`, `${event} Teams`);
}
